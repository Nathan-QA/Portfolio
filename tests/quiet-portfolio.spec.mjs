...
test('production, QA and level design are the actual bilingual positioning',async({page})=>{
 await ready(page);await expect(page.locator('h1')).toHaveText('Nathan Tandille');await expect(page.locator('#heroRoles')).toHaveText('Production · QA · Level Design');
 ...
 await expect(page.locator('#heroDesc')).toContainText('J’accompagne les équipes');
 await page.locator('#langBtn').click();await expect(page.locator('#heroDesc')).toContainText('I work alongside teams');
 await expect(page.locator('#heroRoles')).toContainText('Production');
