...
async function ready(page){await page.goto('/');await expect(page.locator('.ux-article-meta')).toHaveCount(3)}
...
test('three-column articles keep large imagery and readable text on desktop',async({page})=>{
 ...
 for(const card of await page.locator('.case-tile').all()){
  ...
  expect(bounds.height).toBeGreaterThanOrEqual(420);rows.push(bounds.y);
  expect(image.width).toBeGreaterThan(330);expect(image.height).toBeGreaterThan(200);
  expect(await card.locator('.case-title').evaluate(el=>parseFloat(getComputedStyle(el).fontSize))).toBeGreaterThanOrEqual(28);
  expect(await card.locator('.case-lead').evaluate(el=>parseFloat(getComputedStyle(el).fontSize))).toBeGreaterThanOrEqual(16);
...
test('contents align below the toolbar without scrolling the background',async({page})=>{
 ...
 await page.locator('.case-title a').first().click();await expect(page.locator('.modal')).toHaveClass(/ux-reading-mode/);
 await page.locator('.ux-toc summary').click();const before=await page.evaluate(()=>scrollY);
 await page.locator('.ux-toc a').nth(2).click();
 await expect.poll(()=>page.locator('#article-section-3').evaluate(el=>Math.abs(el.getBoundingClientRect().top-el.closest('.modal').querySelector('header').getBoundingClientRect().bottom-20))).toBeLessThan(8);
 expect(await page.evaluate(()=>scrollY)).toBe(before);
 await page.locator('.ux-toc-toggle').click();await expect(page.locator('.ux-toc summary')).toBeFocused();
});
...
test('article title focus opens one window and returns to its title',async({page})=>{
 await ready(page);const link=page.locator('.case-title a').nth(1);await link.focus();await page.keyboard.press('Enter');
 await expect(page.locator('.modal--case-study')).toHaveCount(1);await page.keyboard.press('Escape');await expect(link).toBeFocused();
});
test('day and night finish captures with all local images decoded',async({page},info)=>{
 ...
 await capture('opening-day');await capture('projects-day','projects');await capture('articles-day','cases');await capture('skills-day','about');
 await page.locator('#themeBtn').click();await page.evaluate(()=>scrollTo({top:0,behavior:'instant'}));await capture('opening-night');await capture('articles-night','cases');await capture('projects-night','projects');
